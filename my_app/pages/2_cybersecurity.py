...
df = pd.read_csv("data/cyber_incidents.csv", sep="\t", names=["incident_id", "timestamp",	"severity",	"category",	"status", "description"]) 
...
if len(incidents) > 0 and incidents['phishing'].sum() > 0:
    st.info(
        f"Highest phishing spike occurs on day {incidents['phishing'].idxmax()}. "
        "Recommend reviewing email filtering & user awareness training."
    )
else:
    st.warning("No phishing incident data available.")
